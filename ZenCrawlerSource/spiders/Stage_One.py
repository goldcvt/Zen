...
class ExampleSpider(scrapy.Spider):
    ...
    def fetch_gallery(self, response, other_pubs=None):
        ...
        # TODO check here
        if other_pubs and gall.created_at > (datetime.datetime.now() - datetime.timedelta(days=10)):
    ...
    def fetch_article(self, response, other_pubs=None):
        ...
        #TODO pay attention
        if other_pubs and article.created_at > (datetime.datetime.now() - datetime.timedelta(days=10)):
            yield response.follow_all(other_pubs,
                                     callback=self.fetch_article
                                     )
